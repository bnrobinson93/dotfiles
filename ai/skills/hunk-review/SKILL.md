...
Codex note: Hunk talks to a local daemon. Claude may see the daemon directly, but Codex can be blocked by sandbox networking. If `hunk session *` reports no active session while Hunk is visibly running, retry the same command with escalation. A useful prefix rule is:
