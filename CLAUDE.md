...
agg --theme nord --font-size 16 --font-family "Recursive Mono Casual,Symbols Nerd Font Mono" demo.cast ~/Pictures/demo.gif && rm demo.cast
```
...
- **Performance**: the aarch64 Pi that runs the nightly cron lands near 27ms for fish and 92ms for zsh (measured 2026-09-22 on an idle box; the x86 laptop is faster still, and has fewer of these tools absent). `nightly-dotfiles.sh` benchmarks right after a sync and stow, and discards one warm-up start so post-topgrade cache regeneration stays out of the average; a reading well above baseline is real drift, not the benchmark. Benchmark a candidate config under a scratch `XDG_CONFIG_HOME`: copying over `~/.config/fish/...` writes *through* the stow symlinks and edits the `~/.dotfiles` working copy. Kept fast by never running `tool init | source` directly: generated init scripts are cached via `__cache_gen` (`fish/functions/`, keyed on binary mtime), completions lazy-load from `fish_complete_path`, and mise uses shims instead of `activate`. New tool integrations must follow the same pattern, and must cache the init a tool *runs* rather than a bootstrap stub that re-invokes it — starship needs `--print-full-init`, or the cache holds a 67-byte stub and buys nothing.
