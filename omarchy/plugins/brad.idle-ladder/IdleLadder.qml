...
Item {
  ...
  readonly property string dimCommand: root.home + "/.local/bin/idle-dim"
  ...
  function dim(process, argument) {
    console.log("brad idle-ladder: dim " + argument)
  ...
  Timer {
    ...
    onTriggered: {
      ...
      console.log("brad idle-ladder: fade monitor re-armed at " + fadeMonitor.timeout + "s")
    }
  }
  ...
  IdleMonitor {
    ...
    onIsIdleChanged: {
      if (!isIdle) return
      console.log("brad idle-ladder: suspending after " + root.suspendTimeoutSeconds + "s idle on battery")
  ...
  function logTimeouts() {
    console.log("brad idle-ladder: fade at " + root.fadeTimeoutSeconds + "s, lock at " + root.lockSeconds
