...
def draw_session_label(draw_data: DrawData, screen: Screen, session: str) -> None:
    ...
    mauve = as_rgb(MAUVE)
    # kitty sets the cursor to the first tab's colors before calling draw_tab, and
    # draw_tab_with_powerline reads them back as that tab's bg. Leaving the pill's
    # colors behind paints the first separator in the default fg — a white wedge.
    ...
    # gap before the first tab, on the bar background
    screen.cursor.fg = 0
    screen.cursor.bg = 0
    screen.draw(" ")
