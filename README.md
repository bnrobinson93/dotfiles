...
Files and scripts:
- ssh/config:1 — stowable SSH config enabling AddKeysToAgent, UseKeychain, and IdentityFile defaults
- git/config:1 — enables SSH signing in Git with ssh-keygen + allowed_signers
- jj/config.toml:73 — uses `ssh-sign-wrapper.sh` for signing; defaults to ssh-keygen unless `USE_1PASSWORD_SSH=1`
- dot-local/bin/ssh-setup-github.sh:1 — create/load key and upload to GitHub via `gh`
- dot-local/bin/op-ssh-migrate.sh:1 — migrate key(s) from 1Password to ~/.ssh and agent/keychain
